...
const TARGET_LOCALES: LocaleCode[] = ["en", "fr", "de", "es", "pt", "zh", "ja", "ko"];

// Body: { texts: Record<string, string> } — keys are field names, values are Italian source texts.
// Returns: { translations: Record<string, Record<LocaleCode, string>> }
export async function POST(request: Request) {
  const session = await auth();
  if (!session) return NextResponse.json({ error: "Non autenticato" }, { status: 401 });

  const body = await request.json().catch(() => null) as { texts?: Record<string, string> } | null;
...
Translate the following Italian text fields into: ${TARGET_LOCALES.map(l => LOCALE_NAMES[l]).join(", ")}.
